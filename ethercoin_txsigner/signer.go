...
func (singer *TransactionSigner) SignTransactionHash(msg []byte, privateKey []byte, eccType uint32) ([]byte, error) {
	sig, err := EteSignature(privateKey, msg)
	if err != owcrypt.SUCCESS {
		return nil, fmt.Errorf("ETE sign hash failed")
	}
	return sig, nil
}
...
the second part
*/
func EteSignature(prikey []byte, hash []byte) ([]byte, uint16) {
	signature := make([]byte, 65)
	//	var recid byte
	var ret uint16
	var counter uint32
