...
func (this *ETEBLockScanner) newBlockNotify(block *EthBlock, isFork bool) {
...
func (this *ETEBLockScanner) ScanBlockTask() {
	...
	for {
		...
		if err != nil {
			this.wm.Log.Errorf("get max height of ete failed, err=%v", err)
			break
		}
...
func (this *ETEBLockScanner) newExtractDataNotify(height uint64, tx *BlockTransaction, extractDataList map[string][]*openwallet.TxExtractData) error {
...
func (this *ETEBLockScanner) MakeSimpleToExtractData(tx *BlockTransaction) (string, []*openwallet.TxExtractData, error) {
	...
	if err != nil {
		this.wm.Log.Errorf("calc tx fee in ete failed, err=%v", err)
		return "", extractDataList, err
	}

	amountVal, err := tx.GetAmountEthString()
	if err != nil {
		this.wm.Log.Errorf("calc amount to ete decimal failed, err=%v", err)
		return "", extractDataList, err
	}
...
//extractETETransaction 提取ETE主币交易单
func (this *ETEBLockScanner) extractETHTransaction(tx *BlockTransaction, isTokenTransfer bool) (map[string]*openwallet.TxExtractData, error) {
...
func (this *ETEBLockScanner) extractERC20Transaction(tx *BlockTransaction, contractAddress string, tokenEvent []*TransferEvent) (map[string]*openwallet.TxExtractData, error) {
...
//extractERC20Detail
func (this *ETEBLockScanner) extractERC20Detail(tx *BlockTransaction, contractAddress string, tokenEvent []*TransferEvent, isInput bool, extractData map[string]*openwallet.TxExtractData) ([]string, error) {
