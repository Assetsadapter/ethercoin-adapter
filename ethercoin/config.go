...
func (this *WalletManager) LoadAssetsConfig(c config.Configer) error {
	...
	////钥匙备份路径
	//this.Config.KeyDir = c.String("KeyDir") //filepath.Join(rootDir, "ete", "key")
	////地址导出路径
	//this.Config.AddressDir = c.String("AddressDir") //filepath.Join(rootDir, "ete", "address")
	//区块链数据
	//blockchainDir = filepath.Join(rootDir, strings.ToLower(SymbolID), "blockchain")
	//配置文件路径
	//this.Config.ConfigFilePath = c.String("ConfigFilePath") //ConfigFilePath //filepath.Join(rootDir, "ete", "conf") //filepath.Join("conf")
	////配置文件名
	//this.Config.ConfigFileName = c.String("ConfigFileName") //"ete.ini"
	//区块链数据文件
	...
	//本地数据库文件路径
	//this.Config.DbPath = c.String("DbPath") //filepath.Join(rootDir, "ete", "db")
	//备份路径
	//this.Config.BackupDir = c.String("BackupDir") //filepath.Join(rootDir, "ete", "backup")
	//钱包服务API
	...
	//	this.ChainId = 12
	//this.Config.EthercoinKeyPath = c.String("EthercoinKeyPath") //"/Users/peter/workspace/bitcoin/wallet/src/github.com/ethercoin/go-ethercoin/chain/keystore"
	//每次都向节点查询nonce
...
func (this *WalletConfig) LoadConfig(configFilePath string, configFileName string) (*WalletConfig, error) {
	...
	//钥匙备份路径
	this.KeyDir = c.String("KeyDir") //filepath.Join(rootDir, "ete", "key")
	//地址导出路径
	this.AddressDir = c.String("AddressDir") //filepath.Join(rootDir, "ete", "address")
	//区块链数据
	//blockchainDir = filepath.Join(rootDir, strings.ToLower(SymbolID), "blockchain")
	//配置文件路径
	this.ConfigFilePath = c.String("ConfigFilePath") //ConfigFilePath //filepath.Join(rootDir, "ete", "conf") //filepath.Join("conf")
	//配置文件名
	this.ConfigFileName = c.String("ConfigFileName") //"ete.ini"
	//区块链数据文件
	...
	//本地数据库文件路径
	this.DbPath = c.String("DbPath") //filepath.Join(rootDir, "ete", "db")
	//备份路径
	this.BackupDir = c.String("BackupDir") //filepath.Join(rootDir, "ete", "backup")
	//钱包服务API
	...
	//	this.ChainId = 12
	this.EthercoinKeyPath = c.String("EthercoinKeyPath") //"/Users/peter/workspace/bitcoin/wallet/src/github.com/ethercoin/go-ethercoin/chain/keystore"
	//每次都向节点查询nonce
	localnonce, err := c.Bool("LocalNonce")
	if err != nil {
		log.Error("LocalNonce error, err=", err)
...
//	c.EthercoinKeyPath = "/Users/peter/workspace/bitcoin/wallet/src/github.com/ethercoin/go-ethercoin/chain/keystore"
//	c.ChainID = 12
//	this.Config = &c
//	//创建目录
//	file.MkdirAll(c.DbPath)
